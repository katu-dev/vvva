...
### `simulate_race` — Simulation complète

```python
def simulate_race(self, circuit_id: int, weather: str, year: int = 2024) -> pd.DataFrame:
    # Les annotations ': int', ': str', '-> pd.DataFrame' sont optionnelles, elles indiquent les types

    # ── Étape 1 : Récupère l'historique du circuit ──
    circuit_history = self.data_loader.get_circuit_history(circuit_id, limit=20)
    # Prend les 20 dernières courses sur ce circuit

    # ── Étape 2 : Trouve les pilotes actifs cette année ──
    year_races = self.data_loader.races[self.data_loader.races['year'] == year]

    # Si cette année n'existe pas dans les données, prendre l'année la plus proche
    if len(year_races) == 0:
    ...
    # Récupère les résultats de toutes les courses de cette année
    year_results = self.data_loader.results[
        self.data_loader.results['raceId'].isin(year_races['raceId'])
    ]

    # ── Étape 3 : Liste les pilotes actifs (sans doublons) ──
    ...
    # .get(weather, défaut) = si la météo n'existe pas, utilise 'sunny' par défaut

    # ── Étape 4 : Calcule la performance de chaque pilote ──
    num_drivers = min(len(active_drivers), 24)  # Maximum 24 pilotes sur une grille

    results = []                                # Liste vide qui va accumuler les résultats
    for _, driver in active_drivers.head(num_drivers).iterrows():
        # .iterrows() = parcourt le tableau ligne par ligne
        # _ = on ignore l'index (convention Python : _ = variable qu'on n'utilise pas)

        driver_id = driver['driverId']

        # Forme récente du pilote (jusqu'à l'année choisie)
        recent_form = self.data_loader.get_recent_form(driver_id, year=year)

        # Performance sur ce circuit en particulier
        ...
        if len(driver_circuit_history) > 0:
            ...
            if pd.isna(circuit_perf):       # pd.isna() vérifie si c'est NaN (valeur vide)
                circuit_perf = 10           # Position neutre par défaut
        else:
            circuit_perf = 10               # Pas d'historique → position neutre

        circuit_skill = 1 - (circuit_perf / 20)
        # Même formule que get_recent_form : transforme une position en score 0-1

        # ── Formule de performance finale ──
        performance = (recent_form * 0.7 + circuit_skill * 0.3)
        # 70% basé sur la forme récente, 30% sur l'historique du circuit
        performance *= weather_data['speed']
        # Multiplie par le facteur météo (ex: 0.85 si pluie)
        performance += np.random.normal(0, 0.08)
        # Ajoute un bruit aléatoire : moyenne 0, écart-type 0.08
        # → simule l'imprévisibilité d'une vraie course

        results.append({
            'driver':       f"{driver['forename']} {driver['surname']}",
            'code':         driver['code'],          # Ex: "HAM", "VER"
            'team':         driver['name'],          # Nom de l'équipe
            'performance':  max(0, performance),     # Pas de performance négative
            'grid_position': len(results) + 1        # Position de départ = ordre dans la liste
        })

    # ── Étape 5 : Trie et attribue les positions finales ──
    df = pd.DataFrame(results)
    df = df.sort_values('performance', ascending=False).reset_index(drop=True)
    # Trie du plus performant au moins performant
    # ascending=False → décroissant (le plus grand d'abord)
    # reset_index(drop=True) → remet les index à 0, 1, 2... (drop=True : ne garde pas l'ancien index)

    df['final_position'] = df.index + 1
    # L'index commence à 0, donc +1 pour avoir 1, 2, 3...

    return df[['final_position', 'driver', 'code', 'team', 'grid_position', 'performance']]
    # Retourne uniquement les colonnes utiles, dans un ordre logique
```
